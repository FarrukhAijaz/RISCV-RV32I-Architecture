...
def convert_to_signed(number, bit_length):
    # Mask to isolate the relevant bits
    mask = (1 << bit_length) - 1
    number &= mask
    
    # Check if the number is meant to be signed by examining the MSB
    msb = 1 << (bit_length - 1)
    if number & msb:
        # If MSB is set, convert to signed by subtracting 2^bit_length
        number -= (1 << bit_length)
    return number
class type_r:
